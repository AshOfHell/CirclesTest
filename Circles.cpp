// CADEX.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main()
{
    ...
    {
        vArray.push_back(new Circle(Radi(gen)));
        cout << vArray.at(i)->Radius << " "; 
    }
    uniform_int_distribution<> RDNum(0, ArraySize);
    int RDNumber = RDNum(gen);
    float* Coordinates = vArray.at(RDNumber)->GetCoordinates(AngleRad, vArray.at(RDNumber)->Radius);
    ...
    {
        if (vArray.at(i)->Radius < 50)
        {
            vArray2.push_back(vArray.at(i));
        }
    }
